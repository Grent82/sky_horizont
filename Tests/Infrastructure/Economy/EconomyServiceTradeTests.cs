...
public class EconomyServiceTradeTests
{
    ...
    private static Planet CreatePlanet(Guid id, Guid systemId, Guid factionId, IPlanetRepository repo, ICharacterRepository chars, IPlanetEconomyRepository ecoRepo)
    {
        var planet = new Planet(id, $"P{id.ToString()[..4]}", systemId, factionId,
            new Resources(0, 0, 0), chars, repo, ecoRepo, infrastructureLevel: 0);
        repo.Save(planet);
        return planet;
    }
